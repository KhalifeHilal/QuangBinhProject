...
public sealed class TerrainFloodSimulation : MonoBehaviour
{
    ...
    void FitToReferenceTerrain()
    {
        ...
        worldSize = new Vector2(Mathf.Max(.1f, bounds.size.x * terrainCoverage),
            Mathf.Max(.1f, bounds.size.z * terrainCoverage));
        transform.SetPositionAndRotation(new Vector3(bounds.center.x, transform.position.y, bounds.center.z), Quaternion.identity);
        Debug.Log($"Flood fitted to {referenceTerrainName}: center=({bounds.center.x:F3}, {bounds.center.z:F3}), size=({worldSize.x:F3}, {worldSize.y:F3}). Flow: world left to right (+X).", this);
    }
    ...
    public void RefreshColliderObstacles()
    {
        ...
        foreach (Collider obstacle in colliders)
        {
            ...
            for (int z = z0; z <= z1; z++)
            for (int x = x0; x <= x1; x++) pixels[z * resolution + x] = 255;
        }
